...
\end{problem} 
\begin{solution} 
Метрика --- расстояние между двумя элементами метрического пространства. А метрическое пространство это множество, в котором определено расстояние между любой парой элементов. 
Более формально это выглядит таким образом: Метрическое пространство - это пара $(M, \rho)$, где $M$ -- множество, а $\rho(\cdot, \cdot) : M \times M \to \mathbb{R}^{+}$ какая-то неотрицательная функция, для которой должны быть выполненеы следующие 3 аксиомы, чтобы можно было назвать ее метрикой:
\begin{numerize}
\item $\rho(x,y) = 0 \Leftrightarrow x = y$
\item $\rho(x,y) = \rho(y,x) \forall x,y \in M$
\item $\rho(x,z) \le \rho(x,y) + \rho(y,z) \forall x,y,z \in M $
\end{numerize}
Теперь, когда мы знаем определение метрики легко проверить эти 3 условия для заданных функций.
1. Первая аксиома выполняется автоматически. Вторая аксиома доказывается методом пристального взгляда. А вот третью аксиому уже нужно доказывать. Так почти всегда, первые 2 аксиомы доказываются лекго, а третья в основном не очевидна. 
\end{solution}
